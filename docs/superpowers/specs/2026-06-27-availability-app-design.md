...
Discriminated unions are modeled with `oneOf` + `discriminator` in the spec. openapi-typescript produces proper TS union types. oapi-codegen produces Go wrapper types with type assertions in the repository layer.
...
PATCH  /events/:id                   — host: update settings / dates
                                       client sends desired state, e.g. { visibility: "anonymous" }
                                       server diffs against current state and performs variant swaps
...
- `time_range_start`, `time_range_end`: time-of-day in `HH:mm` format, in the host's timezone.
- `slot` in `availability`: start time of the slot in `HH:mm` format, in the host's timezone. Always zero-padded. Valid slot strings are computed from `time_range_start` + `time_range_end` + `slot_duration_minutes`.
- `date` in `event_dates`: calendar date in `YYYY-MM-DD` format.
- `created_at`: ISO 8601 UTC.

### Timezone Handling

Each event stores the host's IANA timezone (e.g. `Europe/Helsinki`). All time-of-day values are stored in the host's timezone. The frontend defaults to displaying the host's times, with an optional "show in my timezone" toggle that converts client-side using `Intl.DateTimeFormat`. The grid component is timezone-agnostic — it renders whatever times it's given.
...
CREATE UNIQUE INDEX idx_event_dates_id_event_id ON event_dates(id, event_id);
```
...
server/
  ...
  internal/
    handler/      — implements oapi-codegen's ServerInterface
    service/      — business logic, receives repositories as arguments
    repository/   — SQLite queries, one per aggregate
...
- Timezone-agnostic: renders whatever times it receives; conversion happens upstream
